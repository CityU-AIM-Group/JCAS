...
def Class_branch(pred, label_target, CT, input_size_target):
...
def main():
    ...
    for epoch in range(args.num_steps):
        for i, batch in enumerate(targetloader):
            ...
            CT = Class_T()
            loss_seg1 = Class_branch(pred1, label_target, CT, input_size_target)
            loss_seg2 = Class_branch(pred2, label_target, CT, input_size_target)

            #### joint loss ####
            vol_loss = CT.slogdet().logabsdet 

            loss = loss_seg2 + args.lambda_seg * loss_seg1 + 0.5 * vol_loss
            ...
            if (i) % 20 == 0:
                print(
                'epoch = {0:3d}/{1:3d}, iter = {2:3d}, loss_seg1 = {3:.3f} loss_seg2 = {4:.3f} loss_vol = {5:.3f}'.format(
                    epoch, args.num_steps, i, loss_seg1.data.cpu().numpy(), loss_seg2.data.cpu().numpy(), vol_loss.data.cpu().numpy()))
