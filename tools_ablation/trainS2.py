...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = '/home/xiaoqiguo2/Class2affinity/snapshots/Noise_ellipse/Instrument_warmup_epoch92_mIoU50.968.pth'
# RESTORE_FROM = '/home/xiaoqiguo2/Class2affinity/snapshots/Noise_asymmetric50/Instrument_warmup_epoch107_mIoU59.191.pth'
# RESTORE_FROM = '/home/xiaoqiguo2/Class2affinity/snapshots/Noise_symmetric50/Instrument_warmup_epoch112_mIoU58.29.pth'
# RESTORE_FROM = '/home/xiaoqiguo2/OpensetNTM/snapshots/resnet_pretrain.pth'
SAVE_PRED_EVERY = 1
SNAPSHOT_DIR = '../snapshots/Noise_ellipse/'
WEIGHT_DECAY = 0.0005
LOG_DIR = './log/'
...
def class2affinity(transition_matrix):
    ...
    CD = np.load('/home/xiaoqiguo2/Class2affinity/ClassDist/ClassDist_ellipse.npy')
...
def Affinity_branch(feat, label_target, AT, input_size_target):
    ...
    bs, N, _ = affinity.size()
    affinity_map = torch.stack((torch.ones_like(affinity)-affinity, affinity), dim=3)
    affinity_map = affinity_map.contiguous().view(-1, 2)
    affinity_map = torch.mm(affinity_map, AT).view(bs, N, N, 2)[:,:,:,1]
    ...
    loss_aff = F.binary_cross_entropy(affinity_map, affinity_map_label)
...
def Class_branch(pred, label_target, affinity_map, CT, input_size_target):
    ...
    bs, _, h, w = predict.size()
    predict = F.softmax(predict, dim=1).permute(0, 2, 3, 1).contiguous().view(-1, args.num_classes)
    predict = torch.mm(predict, CT).view(bs, h, w, args.num_classes).permute(0, 3, 1, 2)
    loss_seg = seg_loss_woSoftmax(predict, label_target) 

    pred = interp_target(pred)
    # bs, _, h, w = pred.size()
    pred = F.softmax(pred, dim=1).permute(0, 2, 3, 1).contiguous().view(-1, args.num_classes)
    pred = torch.mm(pred, CT).view(bs, h, w, args.num_classes).permute(0, 3, 1, 2)
...
def main():
    ...
    for epoch in range(args.num_steps):
        for i, batch in enumerate(targetloader):
            ...
            loss = loss_seg2 + args.lambda_seg * loss_seg1 + loss_aff1  + args.lambda_seg * loss_aff2 + 0.5 * vol_loss + 0.01 * class_aff_simi
