...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, num_classes, affinity = False):
        self.inplanes = 64
        ...
        if self.affinity:
            self.layer_feat1 = self._make_pred_layer(Classifier_Module, 1024, [6, 12, 18, 24], [6, 12, 18, 24], 128)
            self.layer_feat2 = self._make_pred_layer(Classifier_Module, 2048, [6, 12, 18, 24], [6, 12, 18, 24], 256)
    ...
    def forward(self, x):
        ...
        if self.affinity: 
            ...
            feat1 = self.layer_feat1(x)
            feat2 = self.layer_feat2(feat)
            return feat1, feat2, x1, x2
        else:
            return x1, x2
    ...
    def get_10x_lr_params(self):
        ...
        if self.affinity: 
            b.append(self.layer_feat1.parameters())
            b.append(self.layer_feat2.parameters())
...
def DeeplabMulti(num_classes=4, affinity = False):
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], num_classes, affinity = affinity)
    return model
...
class sig_NTM(nn.Module):
    def __init__(self, num_classes, open_classes=0, init = None):
        ...
        T = torch.ones(num_classes, num_classes)
        ...
        self.Identity_prior = torch.eye(num_classes, num_classes)
        Class_dist = np.load('../ClassDist_ignet.npy')
        self.Class_dist = torch.FloatTensor(np.tile(Class_dist, (num_classes, 1)))
...
class NTM(nn.Module):
    ...
    def forward(self):
        sig = torch.sigmoid(self.w).cuda()
        T = self.identity.detach() + sig * self.co.detach() 
        # if T.shape[0] != 2:
        #     T = T.mul(self.Class_dist.cuda().detach())
        T = F.normalize(T, p=1, dim=1)
        return T